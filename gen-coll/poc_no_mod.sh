...
while true; do
  ...
  # if many restarts happened, regenerate upper paths
  updir=upper_${N}_${dataupper}
  if [ $((++RESTARTS%6)) -eq 0 ]; then datalower=160000; rm -rf $updir; fi
...
###################### FIRST NEAR-COLLISION BLOCK PAIR #####################
  ...
  date
  timeout -s INT 60 $HELPER $diff --findcollision --inputfile1 data/bestpath.bin.gz 2>&1 | tee logs/collfind.log

  RET=$?
  date
  echo $RET
  if [ $RET -eq 124 ]; then
    echo "Command timeout. Restarting"
    sleep 2
    continue
  fi

  # Use only the first collision if there are multiple
  COLLS1=($(ls data/coll1_* | sort))
  COLLS2=($(ls data/coll2_* | sort))
  rm -f "${COLLS1[@]:1}" "${COLLS2[@]:1}"

  if xxd -g 1 data/coll1_* | tail -n +2 | grep ' 0[0ad] ' > /dev/null ||
     xxd -g 1 data/coll2_* | tail -n +2 | grep ' 0[0ad] ' > /dev/null; then
    echo "Forbidden character found. Restarting"
    sleep 2
    continue
  fi
...
################## SECOND NEAR-COLLISION BLOCK PAIR #########################
  ...
  #find example collision
  date
  timeout -s INT 60 $HELPER $diff --findcollision --inputfile1 data/bestpath.bin.gz 2>&1 | tee logs/collfind2.log
  ...
  if xxd -g 1 data/coll1_* | grep ' 0[0ad] ' > /dev/null ||
     xxd -g 1 data/coll2_* | grep ' 0[0ad] ' > /dev/null; then
    echo "Forbidden character found. Restarting"
    sleep 2
    continue
  fi
  ...
  if [ `md5sum collision1.bin | cut -d' ' -f1` != `md5sum collision2.bin | cut -d' ' -f1` ]; then
    echo "Not a real collision. Restarting"
    sleep 2
    continue
  fi
